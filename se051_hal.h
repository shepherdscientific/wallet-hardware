...
// Tang Nano pins 17/18 route to the BL702 USB bridge — do NOT use those.
  ...
  #define TC_SE_UART_RX_PIN  16   // ESP32-S3 GPIO16 ← TernaryCore UART_TX (pin 38)
#endif
#define TC_SE_UART_BAUD         115200
#define TC_SE_UART_TIMEOUT_MS     5000  // fast ops: AT+INFO, AT+STORE, AT+RAND
#define TC_SE_PUBKEY_TIMEOUT_MS  90000  // secp256k1 point mul on PicoRV32 ~27MHz can take 60s+
#define TC_SE_SIGN_TIMEOUT_MS    90000  // same for ECDSA signing
...
se051_err_t se051_read_object(uint8_t obj_id,
                              ...
                              size_t *out_len);
se051_err_t se051_monotonic_counter_get(uint8_t counter_id, uint32_t *value);
