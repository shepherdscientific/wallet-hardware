...
static_assert(BIP39_WORDLIST_SIZE == 2048,
  "BIP39_WORDLIST_SIZE must be exactly 2048 entries");

const char bip39_wordlist[BIP39_WORDLIST_SIZE][BIP39_WORD_MAX_LEN] = {
  ...
  "please", "pledge", "pluck", "plug", "plunge", "poem",
  "poet", "point", "polar", "pole", "police", "pond",
  ...
  "pudding", "pull", "pulp", "pulse", "pumpkin", "punch",
  "pupil", "puppy", "purchase", "purity", "purpose", "purse",
  "push", "put", "puzzle", "pyramid", "quality", "quantum",
  "quarter", "question", "quick", "quit", "quiz", "quote",
  "rabbit", "raccoon", "race", "rack", "radar", "radio",
  "rail", "rain", "raise", "rally", "ramp", "ranch",
  "random", "range", "rapid", "rare", "rate", "rather",
  "raven", "raw", "razor", "ready", "real", "reason",
  ...
  "report", "require", "rescue", "resemble", "resist", "resource",
  ...
  "situate", "six", "size", "skate", "sketch", "ski",
  ...
  "spin", "spirit", "split", "spoil", "sponsor", "spoon",
  "sport", "spot", "spray", "spread", "spring", "spy",
  "square", "squeeze", "squirrel", "stable", "stadium", "staff",
  "stage", "stairs", "stamp", "stand", "start", "state",
  "stay", "steak", "steel", "stem", "step", "stereo",
  "stick", "still", "sting", "stock", "stomach", "stone",
  "stool", "story", "stove", "strategy", "street", "strike",
  "strong", "struggle", "student", "stuff", "stumble", "style",
  "subject", "submit", "subway", "success", "such", "sudden",
  "suffer", "sugar", "suggest", "suit", "summer", "sun",
  "sunny", "sunset", "super", "supply", "supreme", "sure",
  "surface", "surge", "surprise", "surround", "survey", "suspect",
  "sustain", "swallow", "swamp", "swap", "swarm", "swear",
  "sweet", "swift", "swim", "swing", "switch", "sword",
  "symbol", "symptom", "syrup", "system", "table", "tackle",
  "tag", "tail", "talent", "talk", "tank", "tape",
  "target", "task", "taste", "tattoo", "taxi", "teach",
  "team", "tell", "ten", "tenant", "tennis", "tent",
  "term", "test", "text", "thank", "that", "theme",
  "then", "theory", "there", "they", "thing", "this",
  "thought", "three", "thrive", "throw", "thumb", "thunder",
  "ticket", "tide", "tiger", "tilt", "timber", "time",
  "tiny", "tip", "tired", "tissue", "title", "toast",
  "tobacco", "today", "toddler", "toe", "together", "toilet",
  "token", "tomato", "tomorrow", "tone", "tongue", "tonight",
  "tool", "tooth", "top", "topic", "topple", "torch",
  "tornado", "tortoise", "toss", "total", "tourist", "toward",
  "tower", "town", "toy", "track", "trade", "traffic",
  "tragic", "train", "transfer", "trap", "trash", "travel",
  "tray", "treat", "tree", "trend", "trial", "tribe",
  "trick", "trigger", "trim", "trip", "trophy", "trouble",
  "truck", "true", "truly", "trumpet", "trust", "truth",
  "try", "tube", "tuition", "tumble", "tuna", "tunnel",
  "turkey", "turn", "turtle", "twelve", "twenty", "twice",
  "twin", "twist", "two", "type", "typical", "ugly",
  ...
  "upset", "urban", "urge", "usage", "use", "used",
  "useful", "useless", "usual", "utility", "vacant", "vacuum",
  "vague", "valid", "valley", "valve", "van", "vanish",
  "vapor", "various", "vast", "vault", "vehicle", "velvet",
  "vendor", "venture", "venue", "verb", "verify", "version",
  "very", "vessel", "veteran", "viable", "vibrant", "vicious",
  "victory", "video", "view", "village", "vintage", "violin",
  "virtual", "virus", "visa", "visit", "visual", "vital",
  "vivid", "vocal", "voice", "void", "volcano", "volume",
  "vote", "voyage", "wage", "wagon", "wait", "walk",
  "wall", "walnut", "want", "warfare", "warm", "warrior",
  "wash", "wasp", "waste", "water", "wave", "way",
  "wealth", "weapon", "wear", "weasel", "weather", "web",
  "wedding", "weekend", "weird", "welcome", "west", "wet",
  "whale", "what", "wheat", "wheel", "when", "where",
  "whip", "whisper", "wide", "width", "wife", "wild",
  "will", "win", "window", "wine", "wing", "wink",
  "winner", "winter", "wire", "wisdom", "wise", "wish",
  "witness", "wolf", "woman", "wonder", "wood", "wool",
  "word", "work", "world", "worry", "worth", "wrap",
  "wreck", "wrestle", "wrist", "write", "wrong", "yard",
  "year", "yellow", "you", "young", "youth", "zebra",
  "zero", "zone", "zoo"
};
...
uint16_t bip39_word_to_index(const char *word) {
  for (uint16_t i = 0; i < BIP39_WORDLIST_SIZE; i++) {
    if (strcmp(word, bip39_wordlist[i]) == 0) return i;
  }
  return 0xFFFF;
}
...
bool bip39_generate(char wordlist[BIP39_MNEMONIC_WORDS][BIP39_WORD_MAX_LEN]) {
  ...
  if (se051_get_random(entropy, BIP39_ENTROPY_LEN) != SE_OK) {
    secure_zero(entropy, BIP39_ENTROPY_LEN);
    return false;
  }

  sha256(entropy, BIP39_ENTROPY_LEN, hash);

  memcpy(checksum_data, entropy, BIP39_ENTROPY_LEN);
  checksum_data[BIP39_ENTROPY_LEN] = hash[0];
  secure_zero(entropy, BIP39_ENTROPY_LEN);
  secure_zero(hash, SHA256_DIGEST_LENGTH);

  for (int i = 0; i < BIP39_MNEMONIC_WORDS; i++) {
    uint16_t bit_offset = (uint16_t)i * 11;
    uint16_t byte_index = bit_offset / 8;
    uint8_t bit_shift = bit_offset % 8;

    uint16_t index;
    if (bit_shift <= 3) {
      index = ((uint16_t)checksum_data[byte_index] << 8 |
               (uint16_t)checksum_data[byte_index + 1]) >> (5 - bit_shift);
    } else {
      index = ((uint16_t)checksum_data[byte_index] << 16 |
               (uint16_t)checksum_data[byte_index + 1] << 8 |
               (uint16_t)checksum_data[byte_index + 2]) >> (13 - bit_shift);
    }
    index &= 0x7FF;

    memcpy(wordlist[i], bip39_wordlist[index], BIP39_WORD_MAX_LEN);
    wordlist[i][BIP39_WORD_MAX_LEN - 1] = '\0';
  }

  secure_zero(checksum_data, BIP39_CHECKSUM_LEN);
  return true;
}

bool bip39_validate(const char wordlist[BIP39_MNEMONIC_WORDS][BIP39_WORD_MAX_LEN]) {
  uint16_t indices[BIP39_MNEMONIC_WORDS];
  uint8_t raw[BIP39_CHECKSUM_LEN];
  uint8_t hash[SHA256_DIGEST_LENGTH];

  for (int i = 0; i < BIP39_MNEMONIC_WORDS; i++) {
    uint16_t idx = bip39_word_to_index(wordlist[i]);
    if (idx == 0xFFFF) return false;
    indices[i] = idx;
  }

  memset(raw, 0, BIP39_CHECKSUM_LEN);
  for (int i = 0; i < BIP39_MNEMONIC_WORDS; i++) {
    uint16_t bit_offset = (uint16_t)i * 11;
    uint16_t byte_index = bit_offset / 8;
    uint8_t  bit_shift  = bit_offset % 8;

    raw[byte_index] |= (uint8_t)(indices[i] >> (3 + bit_shift));

    uint8_t rem = (uint8_t)(3 + bit_shift);
    if (rem <= 8) {
      raw[byte_index + 1] |= (uint8_t)(indices[i] & ((1U << rem) - 1)) << (8 - rem);
    } else {
      raw[byte_index + 1]  = (uint8_t)((indices[i] >> (rem - 8)) & 0xFF);
      raw[byte_index + 2] |= (uint8_t)(indices[i] & ((1U << (rem - 8)) - 1)) << (16 - rem);
    }
  }

  sha256(raw, BIP39_ENTROPY_LEN, hash);

  uint8_t expected = hash[0];
  uint8_t provided = raw[BIP39_ENTROPY_LEN];

  secure_zero(hash, SHA256_DIGEST_LENGTH);
  secure_zero(raw, BIP39_CHECKSUM_LEN);
