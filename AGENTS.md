...
| `test_bip32` | `g++ -DUSE_SE_STUB -std=c++11 -I. bip32.cpp test_bip32.cpp -o test_bip32` |
...
- Build hash injection: `scripts/post_build.py` replaces `buildhash_plchld` in firmware `.bin` with first 16 hex chars of SHA-256
...
- `prd.json` — 31 user stories (US-001 through US-031), each with title, description, agents, acceptance criteria, priority, passes, notes
...
| System | `watchdog` (TWDT 10s), `version` (build hash placeholder), `fw_integrity` (US-030) |
